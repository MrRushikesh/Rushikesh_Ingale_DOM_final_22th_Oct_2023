...
.then((data) => {
    // console.log(data);
    profileName.innerHTML = `${data[0].uname}`;
    profileEmail.innerHTML = `${data[0].uemail}`;
    profilePhone.innerHTML = `${data[0].uphone}`
    profileStatus.innerHTML = `${sessionStorage.getItem('status')}`
    fetch('http://localhost:5000/reservation',{method:'GET'})
    .then((res) => res.json())
    .then((data) => {
        console.log(data);
        profilePeople.innerHTML = `${data[0].people}`
        profileTime.innerHTML = `${data[0].time}`
        profileDate.innerHTML = `${data[0].date}`
